...
class EventideTeleporter {
  ...
  static getSceneControlButtons(controls) {
    ...
    if (controls.tokens) {
      ...
      if (this.DEBUG)
        console.log("Eventide Teleporter | Added scene control button");
    }
  }
  ...
  static cycleMode() {
    ...
    if (this.DEBUG) {
      console.log(
        "Eventide Teleporter | Cycling from",
        this.mode,
        "to",
        newMode,
      );
  ...
  static applyButtonStyling() {
    requestAnimationFrame(() => {
      ...
      if (!button) {
        if (this.DEBUG)
          console.log("Eventide Teleporter | Button not found for styling");
        return;
      }
      ...
      if (this.DEBUG) {
        console.log(
          "Eventide Teleporter | Applied styling for mode:",
          this.mode,
        );
  ...
  static handleTeleportKeybind(event) {
    ...
    if (!game.user.isGM) {
      ui.notifications.warn(
        game.i18n.localize("EVENTIDE_TELEPORTER.Warnings.GMOnly"),
      );
      return;
    }
    ...
    if (!this.isEnabled) {
      ui.notifications.warn(
        game.i18n.localize("EVENTIDE_TELEPORTER.Warnings.NotEnabled"),
      );
      return;
    }
    ...
    if (selectedTokens.length === 0) {
      ui.notifications.warn(
        game.i18n.localize("EVENTIDE_TELEPORTER.Warnings.NoTokensSelected"),
      );
      return;
    }
    ...
    if (this.DEBUG) {
      console.log(
        "Eventide Teleporter | Teleporting",
  ...
  static async teleportTokens(tokens, position) {
    try {
      if (this.DEBUG) {
        console.log(
          "Eventide Teleporter | Starting teleportation to position:",
          position,
        );
      }
      ...
      for (const token of tokens) {
        ...
        if (this.DEBUG) {
          console.log(
            `Eventide Teleporter | Moving ${token.name} to (${newX}, ${newY})`,
          );
        }
      ...
      if (this.DEBUG)
        console.log(
          "Eventide Teleporter | Teleportation completed successfully",
        );
    } catch (error) {
      console.error("Eventide Teleporter | Error during teleportation:", error);
      ui.notifications.error(
        game.i18n.localize("EVENTIDE_TELEPORTER.Errors.TeleportFailed"),
      );
    }
  }
  ...
  static setupSlowModeListener() {
    ...
    ui.notifications.info(
      game.i18n.localize(
        "EVENTIDE_TELEPORTER.Notifications.SlowModeInstruction",
      ),
    );
  ...
  static snapToGrid(position) {
    if (
      ...
      !game.settings.get(this.MODULE_ID, "snapToGrid")
    ) {
      return position;
    }
